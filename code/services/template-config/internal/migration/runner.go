...
func (r *Runner) createMigrationTable() error {
	query := `
		CREATE TABLE IF NOT EXISTS template_config_migrations (
...
func (r *Runner) getAppliedMigrations() (map[string]*Migration, error) {
	...
	if err := r.db.Raw("SELECT version, name, applied_at, checksum FROM template_config_migrations").Scan(&applied).Error; err != nil {
		return nil, err
	}
...
func (r *Runner) executeMigration(migration *Migration) error {
	...
	if err := tx.Exec(
		"INSERT INTO template_config_migrations (version, name, applied_at, checksum) VALUES (?, ?, ?, ?)",
