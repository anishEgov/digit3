...
docker build -f Dockerfile.migrator -t localization-migrator:latest .

# Build main service image
docker build -t localization:latest .
```
...
spec:
  template:
    spec:
      initContainers:
      - name: migrator
        image: localization-migrator:latest
        env:
        - name: DB_HOST
          value: "postgres"
        - name: DB_PASSWORD
