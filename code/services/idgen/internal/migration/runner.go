...
func (r *Runner) createMigrationTable() error {
	query := `
		CREATE TABLE IF NOT EXISTS idgen_migrations (
...
func (r *Runner) getAppliedMigrations() (map[string]*Migration, error) {
	...
	if err := r.db.Raw("SELECT version, name, applied_at, checksum FROM idgen_migrations").Scan(&applied).Error; err != nil {
		return nil, err
	}
...
func (r *Runner) executeMigration(migration *Migration) error {
	...
	if err := tx.Exec(
		"INSERT INTO idgen_migrations (version, name, applied_at, checksum) VALUES (?, ?, ?, ?)",
