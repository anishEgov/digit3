...
func (r *Runner) getAppliedMigrations() (map[string]*Migration, error) {
	...
	if err := r.db.Raw("SELECT version, name, applied_at, checksum FROM notification_migrations").Scan(&applied).Error; err != nil {
		return nil, err
	}
...
func (r *Runner) executeMigration(migration *Migration) error {
	...
	if err := tx.Exec(
		"INSERT INTO notification_migrations (version, name, applied_at, checksum) VALUES (?, ?, ?, ?)",
